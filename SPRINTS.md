...
# Staff Compliance — Batch A (Canonical Foundation) & Batch B (Secretary Management)

Delivered across two controlled implementation batches plus one dedicated correction/repair round, merged to plugin `main` at `b5f006a66cdcd8df9e127c27da1a94b570b93514` ("feat: add staff compliance foundation") and `3352d300ab8fde56d47d307deacdab4b1f35d64a` ("feat: add secretary staff compliance management"). This is the first implementation of the Staff Compliance direction described in `CLUB_OS_EXPERIENCE_REVIEW_AND_ROADMAP.md`'s Welfare Workspace direction section — **the canonical domain and the Secretary management slice are now implemented; the restricted Welfare projection is not** (see "Known debt / deferred items" below and `DEVELOPMENT_HANDOVER_2026-09-08.md`).
...
- **Restricted Welfare Person projection and Welfare Staff Compliance experience** — not implemented; this is **Staff Compliance Batch C**, the recommended next controlled implementation batch. See `DEVELOPMENT_HANDOVER_2026-09-08.md` and `CLUB_OS_EXPERIENCE_REVIEW_AND_ROADMAP.md`'s Welfare Workspace direction section for the precise expected shape — Welfare must reuse this same canonical `PersonCredentialService`/domain, never a duplicate credential store, and must remain restricted to safeguarding/medical/contact/compliance information appropriate to Welfare (no Finance, no unrelated Secretary administration, no Team-management controls).
- **Compliance expiry alerts** — not implemented; approved future direction (Staff Compliance Batch D), likely a `ComplianceExpiryAlertsProvider` modelled on the established operational alert-provider architecture, using warning/critical semantics (never club-accent/`action_required` semantics for compliance severity).
...
- **`validate-current-emergency-contact.php`** — a pre-existing, unrelated schema/data-version assertion failure, re-confirmed via `git stash` to reproduce identically at the clean Batch-A baseline with none of this work's changes applied. Not caused by, and not repaired by, Staff Compliance.
- All items already carried forward from the Welfare Workspace Workflows batch above (restricted Welfare Person projection scope notes, `validate-visual-foundation.php`, and the items it carried forward in turn) remain unchanged and unresolved by this work.
