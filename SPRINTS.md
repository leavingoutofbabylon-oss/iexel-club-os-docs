...
- **Restricted Welfare Person projection** — not implemented at the time of this batch. No Welfare-scoped Person read route/service existed yet; the Secretary Person Profile was deliberately not exposed to Welfare. **Since implemented as Staff Compliance Batch C** (`cd871f3`) — see "Staff Compliance — Batch A, Batch B & Batch C" below and `DEVELOPMENT_HANDOVER_2026-09-08B.md` for the current state.
- **Staff Compliance** — not implemented at the time of this batch; approved future product direction only. **Since implemented across Batches A, B and C** (`b5f006a6`, `3352d300`, `cd871f3`) — see "Staff Compliance — Batch A, Batch B & Batch C" below.
...
# Staff Compliance — Batch A, Batch B & Batch C

Delivered across three controlled implementation batches (Batch B including one dedicated correction/repair round), merged to plugin `main` at `b5f006a66cdcd8df9e127c27da1a94b570b93514` ("feat: add staff compliance foundation"), `3352d300ab8fde56d47d307deacdab4b1f35d64a` ("feat: add secretary staff compliance management") and `cd871f38b17e1bee341b61e2b5bcba6c7e1f0357` ("feat: add welfare people and compliance workspace"). This is the full implementation of the Staff Compliance direction described in `CLUB_OS_EXPERIENCE_REVIEW_AND_ROADMAP.md`'s Welfare Workspace direction section — **the canonical domain, the Secretary management slice, and the restricted Welfare People/Person/Compliance projection are all now implemented; only compliance expiry alerts (Staff Compliance Batch D) remain future direction** (see "Known debt / deferred items" below and `DEVELOPMENT_HANDOVER_2026-09-08B.md`).
...
- **Compliance expiry alerts** — not implemented; approved future direction (Staff Compliance Batch D), likely a `ComplianceExpiryAlertsProvider` modelled on the established operational alert-provider architecture, using warning/critical semantics (never club-accent/`action_required` semantics for compliance severity). Still the next recommended Staff Compliance batch after Batch C (see below).
...
- **Compliance expiry alerts** — not implemented; approved future direction, **Staff Compliance Batch D** (`ComplianceExpiryAlertsProvider`, modelled on the established operational alert-provider architecture, warning/critical semantics only, never club-accent/`action_required` for compliance severity). Dashboard actions should eventually link to the real destinations Batch C created: the Secretary compliance route where the active workspace is Secretary, the Welfare Person/Compliance destination where the active workspace is Welfare.
...
- **`validate-current-emergency-contact.php`** — the same pre-existing, unrelated schema/data-version assertion failure carried forward from Batch A/B, re-confirmed via `git stash` to reproduce identically with none of Batch C's changes applied. Not caused by, and not repaired by, Staff Compliance.
