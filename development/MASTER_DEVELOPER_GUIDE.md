...
- ✅ Welfare Workspace Workflows (`5348403`) — Welfare Concern Directory mobile/filter repair (reuses the existing form-field component, no new design system) and a new short operational **Welfare Update** capability (dedicated append-only persistence, server-derived author/timestamp, 1,000-character plain-text limit, no edit/delete/attachments, generic Activity History never receives the operational text). This remains a data-minimisation boundary, not detailed safeguarding case notes — see `CLUB_OS_EXPERIENCE_REVIEW_AND_ROADMAP.md`'s Welfare Workspace direction section and `SPRINTS.md` for full delivery detail. At the time of this commit, restricted Welfare Person access and Staff Compliance remained approved future direction, not implemented — **Staff Compliance's canonical foundation and Secretary management have since been implemented** (`b5f006a6`, `3352d300`; see `CLUB_OS_EXPERIENCE_REVIEW_AND_ROADMAP.md`'s Welfare Workspace direction section, `SPRINTS.md`'s "Staff Compliance — Batch A & Batch B" section, and `development/DEVELOPMENT_HANDOVER_2026-09-08.md` for the current resume point). A restricted Welfare Person projection remains not yet implemented.
