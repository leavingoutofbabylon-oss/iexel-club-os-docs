...
| OS-010 | Welfare | Expand into broader compliance workspace — the canonical Staff Compliance credential foundation, Secretary management experience, and the Welfare-facing restricted People/Person/Compliance projection are all now implemented (`b5f006a6`/`3352d300`/`cd871f3`; DBS/safeguarding/first-aid/qualification categories, type-aware naming, target-Person eligibility, Welfare People directory + profile + compliance management). Only compliance expiry alerts (Staff Compliance Batch D) remain future direction. See `SPRINTS.md`'s "Staff Compliance — Batch A, Batch B & Batch C" section | Resolved gap (compliance foundation + Secretary + Welfare) | Complete (Batch D expiry alerts remain future) | MVP / Post-MVP (Batch D only) |
...
**Staff Compliance — canonical foundation, Secretary management, and the restricted Welfare People/Person/Compliance projection are all now complete** (plugin `main` `b5f006a6`, `3352d300`, then `cd871f3`) — see the Staff Compliance bullets above and `SPRINTS.md`'s "Staff Compliance — Batch A, Batch B & Batch C" section for full delivery detail. **Do not restart or re-scope this work; do not re-implement the credential domain, either workspace's route/workflow, target-Person eligibility, type-aware credential naming, the desktop layout repair, or the Welfare People inclusion rule/directory/profile.** Compliance expiry alerts (**Staff Compliance Batch D**) remain the next, separately-schedulable future step — a `ComplianceExpiryAlertsProvider` modelled on the established operational alert-provider architecture, warning/critical semantics only (never club-accent/`action_required` for compliance severity), with dashboard actions eventually able to link to the real destinations Batch C created (Secretary's route where the active workspace is Secretary, Welfare's Person/Compliance destination where the active workspace is Welfare).

The recommended next implementation batch is **Staff Compliance Batch D — Compliance Expiry Alerts** (see above) — not yet started; do not begin it without a fresh confirmation from the Product Owner/lead developer. A small, genuinely open data-integrity item was found during the Event Builder batch (Person 176 has an active Team Assignment with no corresponding `people` row) and remains unresolved — see `SPRINTS.md`'s "Known debt / deferred items" for that batch. Before starting Batch D or any other roadmap-labelled implementation batch, see `development/DEVELOPMENT_HANDOVER_2026-09-08B.md` (the current, latest handover) for the current recommended starting point and outstanding Release Readiness work.
