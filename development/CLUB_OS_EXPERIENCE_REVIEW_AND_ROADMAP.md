...
| OS-010 | Welfare | Expand into broader compliance workspace — approved future direction is now precise (shared canonical Staff Compliance credential record with Secretary/Welfare projections, DBS/safeguarding/first-aid/custom qualifications, expiry semantics); see the Welfare Workspace direction section below. Not yet implemented | Future feature | Medium | Post-MVP |
...
- **Restricted Welfare Person projection — future work, not yet implemented:** the Welfare dashboard's Emergency Contacts and Players Requiring Attention rows remain informative-only pending this. It must expose only appropriate safeguarding/welfare-relevant information (identity, club/player context, emergency contact, current Operational Medical & Safety, medical consent/completeness, legitimate Concern linkage) and must never reuse the Secretary Person Profile wholesale or leak Finance, Registration administration, Team Assignment controls or other unrelated Secretary operations.
- **Staff Compliance — approved future product direction, not yet implemented.** Post-MVP, expand beyond Concern management into a single canonical, shared Person/staff credential record with role-appropriate Secretary and Welfare projections (explicitly not two duplicated credential systems) covering DBS, safeguarding, first aid and FA/custom qualifications. Each credential is expected to carry: type/name, issue/award date, an optional expiry/renewal date, an optional reference/registration number where appropriate, a short administrative note, auditability, and a derived status (`Valid`, `Expiring soon`, `Expired`, `No expiry`/equivalent, `Missing/Incomplete` where appropriate) — using semantic colour only (expired = red, expiring soon = amber/warning, valid = the normal success/neutral treatment), never the club accent colour, for severity. The credential type list must remain extensible/custom, not a brittle hard-coded enum. Advance expiry attention should eventually surface in the appropriate Welfare/Secretary priority areas. This is product direction to inform a future controlled batch's source audit and architecture — it is not scaffolded, scheduled or begun.
...
**Welfare Concern Directory/mobile-filter repair and short operational Welfare Updates are now complete** (plugin `main` `5348403`, "feat: improve welfare workspace workflows") — see the Welfare Workspace Workflows bullet above and `SPRINTS.md` for full delivery detail. **Do not restart or re-scope this work.** A temporary interim dashboard action linking Emergency Contacts/Players Requiring Attention to "View concerns" was manually rejected as semantically wrong and removed before this commit — do not reintroduce or document it as delivered. A restricted Welfare Person projection and Staff Compliance (canonical shared credential records with Secretary/Welfare role-appropriate projections; DBS/safeguarding/first-aid/qualifications; expiry semantics) are both approved future product direction and remain genuinely unimplemented — see the Welfare Workspace direction section above for the precise expected shape of each before either is started.

No new Pre-MVP product decision beyond the above is currently confirmed as the next implementation batch. A small, genuinely open data-integrity item was found during the Event Builder batch (Person 176 has an active Team Assignment with no corresponding `people` row) and remains unresolved — see `SPRINTS.md`'s "Known debt / deferred items" for that batch. Before selecting a new roadmap-labelled implementation batch, see `development/DEVELOPMENT_HANDOVER_2026-09-07B.md` (the current, latest handover) for the current recommended starting point and outstanding Release Readiness work.
