...
- **2D-B1 — Registered-only Training Only → Match Player:** Final acceptance passed; pending commit. The Secretary Training Member detail requires exactly one current Person-linked `registered` Registration and a compatible active current TeamSeason, defensively ensures Player identity, creates a new regular active Player assignment episode, ends the TrainingMembership and writes one safe orchestration event in one transaction. Registration, account, family, Finance, Event audiences and history remain unchanged.
- **2D-B2 and historical experience:** Not implemented. Secretary-native Registration setup from a Training Member, historical-only former Player Experience, Parent Preview policy changes and a new Training Member portal remain separate future work.
